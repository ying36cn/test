    	...
    	function cPlugin(o,emlnum){
    		var sjObj=o;
		        ...
		        sjObj.defaults={
		            ...
		            mArr:[],//分钟数组
		            sArr:[],//分钟数组
		            yyyy:dateBox.getFullYear(),//当前显示年
		            mm:"01",//当前显示月
		            dd:"01",//当前显示日
		            h:"01",//当前显示小时
		            m:"01",//当前显示分
		            ...
		            df_box:function(){return $("</div><div class='df-box "+(sjObj.opt.alwaysShow?"alwaysShow":"")+" "+sjObj.opt.boxClassName+"' ></div>")},
...
//		                    	console.log(objDate.getMonth());
		            ...
		            strEnd:function(c){
		                var df=this;
		                var str = "</ul><div class='G-bg'><div class='G-top' style='height:"+(df.height*2)+"px'></div><div class='G-mid' style='height:"+df.height+"px'></div><div class='G-btm' style='height:"+(df.height*2)+"px'></div></div></div>";
		                return str;
		            },
		            //数字小于10 就在字前面+一个0
		            fillZero:function(x){
		            ...
		            bindFun:function(){
		                ...
		                sjObj.opt.timeElm.find(".df-ok").on("click",function(){
		                    var str="";
		                    var dfText = $('.df-final').html();
	                     	//console.log(dfText);
		                    $('.ca_date').html(dfText);
			                   	dfText = dfText.replace('年',',');
			                   	dfText = dfText.replace('月','');
		                   	...
		                   	var arr = new Array();
		                   		...
		                   		paIndex = 1;
		                   		//每次查询清空内容
	                   			$('#money_cont').find('.grant_cont').remove();
	                   			if($(".nav .select-active").attr("ids") == "qiandao"){
	                   				show_list(arr[0],arr[1],paIndex);
	                   			} else{
	                   				strMoney = "";
	                   				$('#list').queryData(arr[0],arr[1],paIndex,defQuery);
		                   			//timeMoney(arr[0],arr[1],paIndex);
		                   		}
		                    if(sjObj.opt.Year) {
		            ...
		            daysJudge:function(name){
		            	var objDate = new Date();
		            	function xxxx(){
		            		var oli = $('.df-ul').eq(1).find('li');
		                    if(parseInt(sjObj.opt.yyyy) == objDate.getFullYear()){
		                    	var showLi = 0;
		                    	$.each(oli, function(index,ele) {
                                    if ($(ele).attr('data-val') > objDate.getMonth() + 1) {
                                        $(ele).addClass('df-hide').removeClass('df-show');
                                    }
                                })
							
		                    }else{
		                    	$.each(oli, function(index,ele) {
		                    		$(ele).addClass('df-show').removeClass('df-hide');
		                    	});
		                    }
						};
...
//		                            }
		        ...
		        function getnyrstr(str) {
			       	...
			       	if(valarr.length == 3) {
			       		...
			       		r = r.replace("dd", valarr[2]=='dd'?'01':valarr[2]);
